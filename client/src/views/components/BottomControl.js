      ...
      .then(isAuthor => {
        ...
        AudioRecorder.onFinished = async (data) => {
          ...
          speechRecognition = async () => {
            let res = await axios.post(config.url + "/speechRecognition", JSON.stringify(data), { headers: { 'Content-Type': 'application/json' } })
            let content = res.data
            return content
          }

          main = async () => {
            let content = await speechRecognition()
            if (content.indexOf('主页') !== -1) {
              navigation.navigate('Home')
            } else if (content.indexOf('视频') !== -1) {
              navigation.navigate('Video')
            } else if (content.indexOf('音乐') !== -1) {
              navigation.navigate('Music')
            } else if (content.indexOf('新闻') !== -1) {
              navigation.navigate('News')
            } else {
              let speechRes = await axios.post(config.url + "/aiChatContent", { content: content })
